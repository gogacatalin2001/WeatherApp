		...
		F3(
			function (x, y, z) {
				return A3(
					...
					$elm$time$Time$millisToPosix((x + utcOffset) * 1000),
					y,
					z);
			}),
		hourlyData.times,
...
var $author$project$View$Day$view = function (dailyData) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						$author$project$Util$Time$formatDate(dailyData.date))
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						function () {
							var _v0 = dailyData.highTemp;
							if (_v0.$ === 'Nothing') {
								return 'unavailable';
							} else {
								return 'H: ' + $elm$core$String$fromFloat(
									A2($elm$core$Maybe$withDefault, 0.0, dailyData.highTemp));
							}
						}())
					])),
				A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						function () {
							var _v1 = dailyData.lowTemp;
							if (_v1.$ === 'Nothing') {
								return 'unavailable';
							} else {
								return 'L: ' + $elm$core$String$fromFloat(
									A2($elm$core$Maybe$withDefault, 0.0, dailyData.lowTemp));
							}
						}())
					])),
				A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						'Total precipitation: ' + $elm$core$String$fromFloat(dailyData.totalPrecipitaion))
					]))
			]));
};
...
var $author$project$View$WeatherChart$ChartData = F6(
	function (now, minTempPoint, maxTempPoint, hourlyPoints, hovering, itemsToShow) {
		return {hourlyPoints: hourlyPoints, hovering: hovering, itemsToShow: itemsToShow, maxTempPoint: maxTempPoint, minTempPoint: minTempPoint, now: now};
	});
var $author$project$View$WeatherChart$showAllItems = {currentTime: true, minMax: true, precipitation: true, temperature: true};
var $author$project$Main$getChartData = function (weatherData) {
	var now = weatherData.time;
	var minTempPoint = A2(
		$author$project$Util$minimumBy,
		function ($) {
			return $.temperature;
		},
		$author$project$Model$WeatherData$toHourlyDataPoints(weatherData.weather));
	var maxTempPoint = A2(
		$author$project$Util$maximumBy,
		function ($) {
			return $.temperature;
		...
		$author$project$Model$WeatherData$toHourlyDataPoints(weatherData.weather));
	var itemsToShow = $author$project$View$WeatherChart$showAllItems;
	var hovering = _List_Nil;
	var hourlyPoints = $author$project$Model$WeatherData$toHourlyDataPoints(weatherData.weather);
	return A6($author$project$View$WeatherChart$ChartData, now, minTempPoint, maxTempPoint, hourlyPoints, hovering, itemsToShow);
};
							...
							return _Utils_Tuple2(
								$author$project$Model$HaveWeatherAndTime(
									_Utils_update(
										data,
										{
											selectedItems: A3($author$project$Model$WeatherItems$set, item, newValue, data.selectedItems)
										})),
								$elm$core$Platform$Cmd$none);
						} else {
							break _v1$4;
...
var $author$project$View$Week$view = function (data) {
	...
	if (!_v0.b) {
		...
		var renderDailyData = F3(
			function (first, last, dataList) {
				if (!dataList.b) {
					return A2($elm$html$Html$div, _List_Nil, _List_Nil);
				} else {
					var d = dataList.a;
					...
					return _Utils_eq(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										...
										'H: ' + $elm$core$String$fromFloat(
											A2($elm$core$Maybe$withDefault, 0, d.highTemp)))
									])),
								A2(
								$elm$html$Html$div,
								_List_fromArray(
								...
								_List_fromArray(
									[
										...
										'L: ' + $elm$core$String$fromFloat(
											A2($elm$core$Maybe$withDefault, 0, d.lowTemp)))
									])),
								A2(
								$elm$html$Html$div,
								_List_fromArray(
								...
								_List_fromArray(
									[
										$elm$html$Html$text(
										'Total precipitation: ' + $elm$core$String$fromFloat(d.totalPrecipitaion))
									]))
							])) : (_Utils_eq(
						_List_fromArray(
							[d]),
						...
						_List_fromArray(
							...
							])) : $author$project$View$Day$view(d));
				}
			});
		var lastDay = A2(
			$elm$core$List$take,
			1,
			$elm$core$List$reverse(data.dailyData));
		var firstDay = A2($elm$core$List$take, 1, data.dailyData);
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
			...
			_List_fromArray(
				[
					A3(renderDailyData, firstDay, lastDay, data.dailyData)
				]));
	}
};
